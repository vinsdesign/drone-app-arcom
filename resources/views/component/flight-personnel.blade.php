    ...
    <div class="mb-4">
        ...
        <div id="main-content-{{ $item->id }}" class="main-content px-2" style="display: none">
            ...
            <div class="flex items-center justify-between py-4 px-6 border-t border-gray-400 bg-gray-300 dark:bg-gray-700  shadow-lg">
                <div class="flex-1 min-w-[180px]">
                    <p class="text-sm text-gray-700 dark:text-gray-400"><strong>{!! TranslationHelper::translateIfNeeded('Personnel:')!!} </strong>{{$item->users->name}} {!! TranslationHelper::translateIfNeeded('(Pilot)')!!} {{$item->instructors->name ?? null}} {!! TranslationHelper::translateIfNeeded('(Instructor)')!!}</p>
                </div>
            </div>
        </div>
    </div>
