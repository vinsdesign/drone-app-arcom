...
if (Auth()->user()->roles()->pluck('name')->contains('super_admin') || Auth()->user()->roles()->pluck('name')->contains('panel_user')) {
    $queryDocument = App\Models\Document::query()->where('equidment_id', $id)->where('status_visible', '!=', 'archived')->get();
} else {
...
$flightsEquidment = App\Models\Fligh::where('teams_id', $teams)
...
$flightsmarge = $flightsEquidment->merge($flightsKits);
        ...
        <div
            ...
            <div class="flex flex-wrap gap-2 justify-center">
                ...
                <button id="tab1"
                    ...
                    {!! TranslationHelper::translateIfNeeded('Equipment Doc') !!}
                </button>
                <button id="tab2"
    ...
    $(document).ready(function() {
        $('#triggerButton').click(function() {
            ...
            if (name == null) {
                ...
                $.ajax({
                    url: '{{ route('create.document.equipment') }}',
                    type: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
